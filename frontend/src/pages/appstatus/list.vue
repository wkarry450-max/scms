            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <template v-if="auth.canView('appstatus/add')">
                                <router-link :to="`/appstatus/add`">
                                    <Button :label="$t('addNewAppstatu')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                                </router-link>
                            </template>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/appstatus'}" :model="pageBreadCrumb" />
                            </template>
                                    ...
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="id" :header="$t('id')" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/appstatus/view/${data.id}`">
                                                    {{ data.id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        <Column  field="app_status" :header="$t('appStatus')" >
                                            <template #body="{ data, index }">
                                                {{ data.app_status }}
                                            </template>
                                        </Column>
                                ...
                                <template v-if="showFooter && pageReady">
                                    <div class="grid justify-content-between align-items-center">
                                        <div class="flex gap-2 flex-grow-0">
                                            <template v-if="auth.canView('appstatus')">
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'appstatuslist',
		},
		apiPath: {
			type: String,
			default: 'appstatus/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/appstatus/view/${data.id}`,
			icon: "pi pi-eye",
			visible: auth.canView('appstatus/view')
		},
		{
			label: () => $t('edit'),
			to: `/appstatus/edit/${data.id}`,
			icon: "pi pi-pencil",
			visible: auth.canView('appstatus/edit')
		},
		{
			label: () => $t('delete'),
			command: (event) => { deleteItem(data.id) },
			icon: "pi pi-trash",
			visible: auth.canView('appstatus/delete')
		}
	]
	}
	
	onMounted(()=>{ 
		const pageTitle = $t('appstatus');
