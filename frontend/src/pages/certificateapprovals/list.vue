                    ...
                    <div class="grid justify-content-between align-items-center">
                        <div  class="col " >
                            <div class=" text-2xl text-primary font-bold" >
                                {{ $t('certificateapprovals') }}
                            </div>
                        </div>
                        <div  class="col-fixed " >
                            <template v-if="auth.canView('certificateapprovals/add')">
                                <router-link :to="`/certificateapprovals/add`">
                                    <Button :label="$t('addNewCertificateapproval')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                                </router-link>
                            </template>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/certificateapprovals'}" :model="pageBreadCrumb" />
                            </template>
                            <!-- page records template -->
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                     :value="records" dataKey="approval_id" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="approval_id" :header="$t('approvalId')" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/certificateapprovals/view/${data.approval_id}`">
                                                    {{ data.approval_id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        ...
                                        <Column  field="decision" :header="$t('decision')" >
                                            <template #body="{ data, index }">
                                                {{ data.decision }}
                                            </template>
                                        </Column>
                                        <Column  field="comments" :header="$t('comments')" >
                                            <template #body="{ data, index }">
                                                {{ data.comments }}
                                            </template>
                                        </Column>
                                        <Column  field="decided_at" :header="$t('decidedAt')" >
                                            <template #body="{ data, index }">
                                                {{ data.decided_at }}
                                            </template>
                                        </Column>
                                ...
                                <template v-if="showFooter && pageReady">
                                    <div class="grid justify-content-between align-items-center">
                                        <div class="flex gap-2 flex-grow-0">
                                            <template v-if="auth.canView('certificateapprovals')">
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'certificateapprovalslist',
		},
		apiPath: {
			type: String,
			default: 'certificateapprovals/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/certificateapprovals/view/${data.approval_id}`,
			icon: "pi pi-eye",
			visible: auth.canView('certificateapprovals/view')
		},
		{
			label: () => $t('edit'),
			to: `/certificateapprovals/edit/${data.approval_id}`,
			icon: "pi pi-pencil",
	...
	onMounted(()=>{ 
		const pageTitle = $t('certificateapprovals');
