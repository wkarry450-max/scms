            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <template v-if="auth.canView('certificatenews/add')">
                                <router-link :to="`/certificatenews/add`">
                                    <Button :label="$t('addNewCertificatenew')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                                </router-link>
                            </template>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/certificatenews'}" :model="pageBreadCrumb" />
                            </template>
                            <!-- page records template -->
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                     :value="records" dataKey="news_id" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="news_id" :header="$t('newsId')" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/certificatenews/view/${data.news_id}`">
                                                    {{ data.news_id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        ...
                                        <Column  field="published_by" :header="$t('publishedBy')" >
                                            <template #body="{ data, index }">
                                                {{ data.published_by }}
                                            </template>
                                        </Column>
                                        <Column  field="published_at" :header="$t('publishedAt')" >
                                            <template #body="{ data, index }">
                                                {{ data.published_at }}
                                            </template>
                                        </Column>
                                ...
                                <template v-if="showFooter && pageReady">
                                    <div class="grid justify-content-between align-items-center">
                                        <div class="flex gap-2 flex-grow-0">
                                            <template v-if="auth.canView('certificatenews')">
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'certificatenewslist',
		},
		apiPath: {
			type: String,
			default: 'certificatenews/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/certificatenews/view/${data.news_id}`,
			icon: "pi pi-eye",
			visible: auth.canView('certificatenews/view')
		},
		{
			label: () => $t('edit'),
			to: `/certificatenews/edit/${data.news_id}`,
			icon: "pi pi-pencil",
			visible: auth.canView('certificatenews/edit')
		},
		{
			label: () => $t('delete'),
			command: (event) => { deleteItem(data.news_id) },
			icon: "pi pi-trash",
			visible: auth.canView('certificatenews/delete')
		}
	]
	}
	
	onMounted(()=>{ 
		const pageTitle = $t('certificatenews');
