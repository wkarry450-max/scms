                    ...
                    <div class="grid justify-content-between align-items-center">
                        <div  class="col " >
                            <div class=" text-2xl text-primary font-bold" >
                                {{ $t('certificatecategories') }}
                            </div>
                        </div>
                        <div  class="col-fixed " >
                            <template v-if="auth.canView('certificatecategories/add')">
                                <router-link :to="`/certificatecategories/add`">
                                    <Button :label="$t('addNewCertificatecategory')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                                </router-link>
                            </template>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/certificatecategories'}" :model="pageBreadCrumb" />
                            </template>
                                    ...
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="category_id" :header="$t('categoryId')" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/certificatecategories/view/${data.category_id}`">
                                                    {{ data.category_id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        <Column  field="category_name" :header="$t('categoryName')" >
                                            <template #body="{ data, index }">
                                                {{ data.category_name }}
                                            </template>
                                        </Column>
                                ...
                                <template v-if="showFooter && pageReady">
                                    <div class="grid justify-content-between align-items-center">
                                        <div class="flex gap-2 flex-grow-0">
                                            <template v-if="auth.canView('certificatecategories')">
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'certificatecategorieslist',
		},
		apiPath: {
			type: String,
			default: 'certificatecategories/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/certificatecategories/view/${data.category_id}`,
			icon: "pi pi-eye",
			visible: auth.canView('certificatecategories/view')
		},
		{
			label: () => $t('edit'),
			to: `/certificatecategories/edit/${data.category_id}`,
			icon: "pi pi-pencil",
			visible: auth.canView('certificatecategories/edit')
		},
		{
			label: () => $t('delete'),
			command: (event) => { deleteItem(data.category_id) },
			icon: "pi pi-trash",
			visible: auth.canView('certificatecategories/delete')
		}
	]
	}
	
	onMounted(()=>{ 
		const pageTitle = $t('certificatecategories');
