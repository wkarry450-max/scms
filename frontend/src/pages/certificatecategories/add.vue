                                        ...
                                        <div class="col-12">
                                            <div class="formgrid grid">
                                                ...
                                                <div class="col-12 md:col-9">
                                                    <InputText  ref="ctrlcategory_name" v-model.trim="formData.category_name"  :label="$t('categoryName')" type="text" :placeholder="$t('enterCategoryName')"      
                                                    class=" w-full" :class="getErrorClass('category_name')">
                                                    </InputText>
                                                    <small v-if="isFieldValid('category_name')" class="p-error">{{ getFieldError('category_name') }}</small> 
                                                </div>
                                            </div>
                                        </div>
                                        <div class="col-12">
...
<script setup>
	...
	const props = defineProps({
		...
		routeName : {
			type : String,
			default : 'certificatecategoriesadd',
		},
		apiPath : {
			type : String,
			default : 'certificatecategories/add',
		},
		submitButtonLabel: {
			type: String,
			default: () => $t('submit'),
		...
		pageData: { // use to set formData default values from another page
			type: Object,
			default: () => ({
				category_name: "", description: "", created_at: "CURRENT_TIMESTAMP", 
			})
		},
	});
	//lines
	...
	function afterSubmit(response) { 
		...
		if(app.isDialogOpen()){
			...
			app.navigateTo(`/certificatecategories`);
		}
	}
	...
	onMounted(()=>{
		const pageTitle = $t('addNewCertificatecategory');
