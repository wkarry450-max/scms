                            ...
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                    ...
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        ...
                                        <Column  field="file_path" :header="$t('filePath')" >
                                            <template #body="{ data, index }">
                                                {{ data.file_path }}
                                            </template>
                                        </Column>
                                        <Column  field="approvals_status" :header="$t('approvalsStatus')" >
                                            <template #body="{ data, index }">
                                                {{ data.approvals_status }}
                                            </template>
                                        </Column>
                                        <Column  field="uploaded_at" :header="$t('uploadedAt')" >
                                            <template #body="{ data, index }">
                                                {{ data.uploaded_at }}
                                            </template>
                                        </Column>
                                        <Column  field="users_user_name" :header="$t('usersUserName')" >
                                            <template #body="{ data, index }">
                                                {{ data.users_user_name }}
                                            </template>
                                        </Column>
...
<script setup>
	...
	function getActionMenuModel(data){
		...
		},
		{
			...
			icon: "pi pi-pencil",
			visible: auth.canView('certificates/edit')
		},
		{
			label: () => $t('delete'),
			command: (event) => { deleteItem(data.certificate_id) },
			icon: "pi pi-trash",
			visible: auth.canView('certificates/delete')
		}
	]
	}
