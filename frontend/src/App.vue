<template>
    <div :class="containerClass" @click="onWrapperClick">
        <template v-if="auth.isLoggedIn">
            <div class="layout-topbar bg-pink-50 shadow-7">
                ...
                <router-link to="/" class="layout-topbar-logo flex-grow-none">
                    ...
                    <span class="text-purple-50">{{ $appName }}</span>
                </router-link>
                    ...
                    <div class="flex">
                        <template v-for="(item, index) of navbarTopLeftItems" :key="`navtopleftmenu-${index}`">
                            <router-link :to="item.to">
                                <Button :label="item.label" :icon="item.icon" class="p-button-text text-purple-50 page-button" />
                            </router-link>
                        </template>
                    </div>
                    <SplitButton class="layout-menu-user-button"  icon="pi pi-user" :model="appBarActionMenu">
                        <router-link to="/account">
                            <Avatar shape="circle" class="bg-pink-50 text-purple-50" size="large" icon="pi pi-user" />
                            </router-link>
                        </SplitButton>
                    </div>
                </div>
                ...
                <template v-else>
                    <div class="layout-topbar bg-pink-50 text-purple-50 shadow-7">
